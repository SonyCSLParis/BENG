...
(defun write-verb-lex-cxn (out lemma &optional verb-type meaning)
  ...
  (unless meaning
    (setf meaning lemma))
  (format out "~%(add-categories '(~a-1 ~a-2) *fusion-hierarchy*)" lemma lemma)
  (format out "~%(add-link '~a-1 'Actor *fusion-hierarchy* :weight 0.2)" lemma)
  (format out "~%(add-link '~a-2 'Undergoer *fusion-hierarchy* :weight 0.2)" lemma)
  (if (eql verb-type 'ergative)
    (format out "~%(add-link '~a-2 'Emphasized-Role *fusion-hierarchy* :weight 0.2)" lemma)
    (format out "~%(add-link '~a-1 'Emphasized-Role *fusion-hierarchy* :weight 0.2)" lemma))   
  (format out "~%~%(def-fcg-cxn ~a-verb-lex" meaning)
  (format out "~%             ((?~a-unit" lemma)
  (format out "~%               (referent ?ref-ev)")
  (format out "~%               (ev-args (?ev ?context))")
  ...
  (format out "~%                        (emphasis ~a))" (if (eql verb-type 'ergative) '?y '?x))
  ...
  (format out "~%               --")
  (format out "~%               (syn-cat (lex-class verb))")
  (format out "~%               (lex-id ~a)))" meaning)
...
(defun write-verb-base-morph (out lemma string &optional meaning)
    ...
    (format out "~%               (footprints (not number morph))")
    (format out "~%               (syn-cat (lex-class verb)")
    (format out "~%                        (verb-form base-form)")
    (format out "~%                        (tam (tense ?tense)")
    (format out "~%                             (aspect (perfect -)")
    (format out "~%                                     (progressive -))")
    (format out "~%                             (modality ?m)))")
...
(defun write-verb-past-tense-form (out lemma string &optional meaning)
  ...
  (format out "~%               (footprints (not number morph))")
  (format out "~%               (syn-cat (lex-class verb)")
  (format out "~%                        (verb-form participle)")
  (format out "~%                        (finite +)")
  (format out "~%                        (agreement ?agr)")
