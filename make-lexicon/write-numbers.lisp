...
(defun write-numbers ()
  (let ((number-words (append (loop for i from 0 to 100
                                    collect (format nil "~r" i))
                              '("thousand" "million" "billion" "trillion" "gazillion"))))
    (dolist (number-word number-words)
      (with-open-file (out (ensure-directories-exist (beng-pathname :directory '("lexicon" "numbers")
                                                                    :name number-word
                                                                    :type "lisp"))
                           :direction :output :if-exists :supersede)
        (add-license-and-copyright-header out)
        (format out "~%~%(in-package :beng)~%")
          (format out "~%(def-fcg-cxn ~a-number-lex" string)
          ...
          (format out "~%               (sem-cat (sem-class cardinal-number))")
          (format out "~%               (syn-cat (lex-class quantifier)")
